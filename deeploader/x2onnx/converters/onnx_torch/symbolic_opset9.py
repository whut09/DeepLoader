...
from . import symbolic_helper as sym_help
from .symbolic_helper import parse_args, _parse_arg, _unimplemented
...
def addmm(g, self, mat1, mat2, beta, alpha):
    return g.op("Gemm", mat1, mat2, self, beta_f=sym_help._scalar(beta),
...
def _reduce_op_symbolic(onnx_op_name, allow_multi_dim_support=True):
    def symbolic(g, self, dim=None, keepdim=None):
        if dim is None:
            ...
            dim, keepdim = sym_help._get_const(dim, desc, 'dim'), sym_help._get_const(keepdim, 'i',
                                                                                      'keepdim')
...
prod = _reduce_with_dtype('ReduceProd', 'prod',
                          allow_multi_dim_support=False)  # torch.prod does not support multidimensional 'dim'
...
def batch_norm(g, input, weight, bias, running_mean, running_var, training, momentum, eps,
               cudnn_enabled):
    ...
    if weight is None or weight.node().mustBeNone():
        ...
        weight_value = torch.tensor([1.] * input_sizes[1])
        weight = g.op("Constant", value_t=weight_value)
    if bias is None or bias.node().mustBeNone():
        assert len(input_sizes) > 1
        # bias_value = torch.tensor([0.] * input_sizes[1]).type(
        #    'torch.' + input.type().scalarType() + 'Tensor')
        bias_value = torch.tensor([0.] * input_sizes[1])
...
def index_fill(g, self, dim, index, value):
    ...
    unsqueezed_index = g.op("Unsqueeze", index,
...
def full(g, sizes, value, dtype, layout, device, pin_memory=False):
    ...
    if sym_help._is_value(const_value):
        ...
        return g.op("ConstantOfShape", sizes,
                    value_t=torch.tensor([const_value],
...
def full_like(g, input, fill_value, dtype, layout, device, pin_memory=False):
    shape = g.op("Shape", input)
    return g.op("ConstantOfShape", shape,
                value_t=torch.tensor([fill_value],
                                     dtype=sym_help.scalar_type_to_pytorch_type[dtype]))
...
def _generic_rnn(g, variant, input, initial_states, all_weights, has_biases,
    ...
    layer_weights = [all_weights[i:i + weights_per_layer] for i in
                     range(0, len(all_weights), weights_per_layer)]
    if batch_first:
    ...
    def reform_weights(g, w, n, intervals):
        slices = [sym_help._slice_helper(g, w, axes=[0], starts=[x * n], ends=[y * n]) for x, y in
                  intervals]
    ...
    def transform_weights(layer_index):
        if variant == 'RNN':
            ...
            weight_ih, weight_hh, bias_ih, bias_hh = \
                [reform_weights(g, w, hidden_size, reform_permutation) for w in
    ...
    def retrieve_state(x, start, end):
        return x if num_layers == 1 else sym_help._slice_helper(g, x, axes=[0], starts=[start],
                                                                ends=[end])
    ...
    for i in range(num_layers):
        ...
        if bidirectional:
            ...
            prev_output = g.op('Reshape', prev_output,
...
def _lstm_full(g, input, hidden_v, weight_v, has_biases, num_layers, dropout, train, bidirectional,
               batch_first):
...
def _lstm_packed(g, input, batch_sizes, hidden_v, weight_v, has_biases, num_layers, dropout, train,
                 bidirectional):
    hidden, weight = sym_help._unpack_list(hidden_v), sym_help._unpack_list(weight_v)
    return _generic_rnn(g, 'LSTM', input, hidden, weight, has_biases, num_layers,
...
def _one_hidden_rnn(kind):
    @parse_args('v', 'v', 'v', 'i', 'i', 'f', 'i', 'i', 'i')
    def _rnn_full(g, input, hidden, weight_v, has_biases, num_layers, dropout, train, bidirectional,
                  batch_first):
    ...
    @parse_args('v', 'v', 'v', 'v', 'i', 'i', 'f', 'i', 'i')
    def _rnn_packed(g, input, batch_sizes, hidden, weight_v, has_biases, num_layers, dropout, train,
                    bidirectional):
        weight = sym_help._unpack_list(weight_v)
        return _generic_rnn(g, kind, input, hidden, weight, has_biases, num_layers,
                            dropout, train, bidirectional, batch_sizes=batch_sizes)
...
def gather(g, self, dim, index, sparse_grad=False):
    ...
    index = g.op("Cast", g.op("OneHot", index, depth, values, axis_i=dim),
...
def arange(g, *args):
    ...
    if len(args) == 5:
        ...
        arange_tensor = g.op("Add",
                             g.op("Squeeze", nonzero(g, ones(g, range_tensor, dtype, *(args[3:]))),
                                  axes_i=[1]), start)
        return g.op("Cast", arange_tensor, to_i=sym_help.scalar_type_to_onnx[dtype])
    elif len(args) == 7:
        ...
        arange_tensor = g.op("Squeeze", nonzero(g, ones(g, range_tensor, dtype, *(args[4:]))),
                             axes_i=[1])
...
def index(g, self, index):
    ...
    if len(indices) == 1:
        ...
        if len(adv_idx_indices) == 0:
            ...
            dim_tensor_list = [
                g.op("Gather", shape_tensor, g.op("Constant", value_t=torch.LongTensor([dim])),
                     axis_i=0) for dim in range(rank)
            ]

            self = g.op("Transpose", self, perm_i=adv_idx_indices + [i for i in range(rank) if
                                                                     i not in adv_idx_indices])
            ...
            if adv_idx_indices == list(range(adv_idx_indices[0], adv_idx_indices[-1] + 1)):
                ...
                folded_adv_idx_shape_list = [g.op("Constant", value_t=torch.LongTensor([-1]))] \
                                            + [dim_tensor_list[i] for i in range(rank) if
                                               i not in adv_idx_indices]
                ...
                adv_idx_permute = list(range(1, adv_idx_indices[0] + 1)) \
                                  + [0] + list(
                    range(adv_idx_indices[0] + 1, rank - adv_idx_count + 1))
                ...
                final_shape_list = [dim_tensor_list[i] for i in range(adv_idx_indices[0])] \
                                   ...
                                   + [dim_tensor_list[i] for i in range(adv_idx_indices[0], rank) if
                                      i not in adv_idx_indices]
...
def gelu(g, self):
    ...
    return mul(g, mul(g, self, erf_plusone),
