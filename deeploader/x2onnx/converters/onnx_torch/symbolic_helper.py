...
def _get_const(value, desc, arg_name):
    if _is_value(value) and value.node().kind() != 'onnx::Constant':
        raise RuntimeError(
            "ONNX symbolic expected a constant value of the {} argument, got `{}`".format(arg_name,
                                                                                          value))
    return _parse_arg(value, desc)
...
def _slice_helper(g, input, axes, starts, ends, steps=None, dynamic_slice=False):
    if _export_onnx_opset_version <= 9:
        from .symbolic_opset9 import _slice
        return _slice(g, input, axes, starts, ends)
    else:
        from .symbolic_opset10 import _slice
...
def _scatter_helper(g, self, dim, index, src):
    if _export_onnx_opset_version <= 10:
        from .symbolic_opset9 import scatter
    else:
        from .symbolic_opset11 import scatter
