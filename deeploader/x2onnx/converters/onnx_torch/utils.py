...
from . import ONNX_ARCHIVE_MODEL_PROTO_NAME, ExportTypes, OperatorExportTypes
...
def _optimize_graph(graph, operator_export_type, _disable_torch_constant_prop=False,
    ...
    if operator_export_type != OperatorExportTypes.RAW:
        ...
        from .symbolic_helper import _export_onnx_opset_version
...
def _trace(func, args, operator_export_type, return_outs=False):
    ...
    trace, torch_out, inputs_states = torch.jit.get_trace_graph(func, args, _force_outplace=True,
...
def _trace_and_get_graph_from_model(model, args, training):
    ...
    with set_training(model, training):
        trace, torch_out, inputs_states = torch.jit.get_trace_graph(model, args,
...
def _export_to_pretty_string(model, args, f, export_params=True, verbose=False, training=False,
                             ...
                             fixed_batch_size=False):
    from .symbolic_helper import _default_onnx_opset_version, _set_opset_version
    from .symbolic_helper import _set_operator_export_type
...
def _add_attribute(node, key, value, aten):
    ...
    if m is None:
        raise IndexError((
                                 ...
                                 " must be suffixed with type, e.g. 'dim_i' or 'dims_i'").format(
            key))
...
def _run_symbolic_function(g, n, inputs, env, operator_export_type=OperatorExportTypes.ONNX):
    ...
    # the new graph
    try:
        from .symbolic_helper import _export_onnx_opset_version as opset_version
        ...
        if ns == "onnx":
            ...
            if op_name == "Constant" and not n.mustBeNone():
                ...
                new_node = new_op_outputs[
                    0].node() if n.outputsSize() > 1 else new_op_outputs.node()
                for b in n.blocks():
                ...
                if not is_exportable:
                    warnings.warn(
                        "ONNX export failed on primitive operator {}; please report a bug".format(
                            op_name))
...
def _validate_dynamic_axes(dynamic_axes, model, input_names, output_names):
    ...
    for key, value in dynamic_axes.items():
        if key not in valid_names:
            warnings.warn(
                "Provided key {} for dynamic axes is not a valid input/output name".format(key))
